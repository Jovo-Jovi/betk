 ...
 * gap goes to Claude Design, not an in-repo patch). The non-invasive
 * composition-layer choice: keep the input usable but redirect `onSend` to a
 * toast stating WHY (the closed-state reason) instead of calling
 * `sendInquiryMessage` — an "attempt → explain" UX, not a silent no-op, and no
 * mutation ever fires for a closed inquiry either way. The caller additionally
 * renders the closed-state banner (`inbox.thread.closedBanner`) above this.
 */
