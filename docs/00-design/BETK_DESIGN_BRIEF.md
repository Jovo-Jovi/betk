...
- **`flag` domain (moderation: pending/reviewed/actioned/dismissed):** tint colors only, **no Arabic/English labels authored** — matches the standing carry-forward (StatusBadge `DEFAULT_LABELS` stays `Partial`). ⚠️ Do NOT author `flag` labels here. **REG-18 KEEP-PARTIAL (decision recorded — sanctioned CD-DELTA-4):** authoring the 4 AR moderation labels now would contradict this locked §5.5/§7 rule, so the decision is to formally defer — `flag` stays label-less with tint colors only; reopen in the admin/moderation phase with brief sign-off.
...
- **Component (addendum — sanctioned CD-DELTA-4):** both styles above ship as one shared `Tabs` wrapping the vanilla `ui/tabs` (Radix Tabs) base via CLI add — keyboard nav, roving focus, and controlled/uncontrolled modes come from the primitive (not hand-rolled). String-prop `{ id, label }` per tab + optional per-tab `count` badge slot; `variant="underline"|"pill"` selects the two styles above; content panels optional (`TabsContent`) for bar-only vs tabbed-panel use. Token-only, RTL-canonical. New dep `@radix-ui/react-tabs`. First consumer: Listings Management T03 status filter tabs (all/active/draft/paused/sold_out/removed).
...
- **Route-transition surfaces (extension — sanctioned CD-DELTA-4, REG-38):** `CatalogSkeletons` adds `CategoryPageSkeleton`, `ListingDetailSkeleton`, and `StorefrontSkeleton` — full-page fallbacks matching each route's final layout for streaming-Suspense on the category / listing-detail / storefront transitions (additive exports; compose the section-level skeletons). **Global route feedback (REG-38b — ADOPTED):** a token-only `RouteProgress` top bar (`fixed inset-x-0 top-0`, `h-3px`, `z-60` above the topbar, indeterminate `--primary` segment, RTL-reversed) mounts only while a navigation is pending; the app drives `active` from router transition state.
