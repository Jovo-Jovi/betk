...
ListingCard, StoreCard, PriceBlock (handles price_type fixed/per_hour/starting_from/quote_only), **StatusBadge** (one map for all enums — keys in `constants/statusColors.ts`), StarRating, RatingSummary, LevelBadge (Bronze/Silver/Gold), VerifiedBadge, MessageThread, ImageUploader, OrderTimeline, AddressForm, FilterSheet, SLABadge, CategoryGrid, CollectionStrip, ImageGallery, StockBadge, SellerMiniCard, SearchBar, FilterChips, WishlistButton, FollowButton, EmptyState, SkeletonGrid/SkeletonTable, ErrorRetryCard, ConfirmDialog, Toaster. Every component must render its empty / loading / error states per `docs/standards/UI_STATE_STANDARDS.md`, not just the happy path.
