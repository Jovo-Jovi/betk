...
**R-S08 retention semantics (Phase 04 / T05).** A rejected application's resubmission uploads to a NEW object path under the same owner-prefix and repoints the `seller_documents.storage_path`; because the `docs` bucket has no UPDATE/DELETE policy, the ORIGINAL ID objects **persist at their old paths** (recoverable by prefix, never garbage-collected in MVP). This is an accepted PII-lifecycle posture, not a leak: admin review (v1 Phase 14 — retired numbering; v2 owner: Phase 09) should know that prior-version ID objects continue to exist at their old paths and account for them in any PII-erasure work.
