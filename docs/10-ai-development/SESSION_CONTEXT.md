...
## Next task
- Phase 01 / T08 (Opus 4.8): RLS verification harness. Write `tests/integration/rls.smoke.test.ts` asserting default-deny and core policies (anon cannot read draft listing, can read active; buyer cannot read another buyer's orders; `my_store_id()`/`is_admin()` resolve; non-phone Google user rejected by transaction-gate; phone-verified user passes). Requires a test Supabase project (not production).
