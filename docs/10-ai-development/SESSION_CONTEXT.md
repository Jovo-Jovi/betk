...
- User must run `supabase link --project-ref <ref>` and populate `.env.local` before T05.

## Next task
- Phase 01 / T05: Database migrations — apply schema from BETK_DATABASE_SCHEMA.sql (43 tables + OD-2/OD-4 deltas from SESSION_CONTEXT). Run `supabase gen types typescript --local > src/lib/supabase/types.ts`.
