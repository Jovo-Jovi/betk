...
- **FR-SEL-3 (Dashboard KPIs) is NOT in this phase.** BETK_PHASES lists FR-SEL-3 under Phase 13 (v1 heading, §H) (snapshots/KPIs) while Phase 04's feature list omits it; the numeric range "FR-SEL-1..7" is reconciled in favor of the explicit feature lists. Phase 04 ships `/seller` as the UI-Spec **empty-state** landing only ("No activity yet — add your first listing" + CTA); Phase 13 (v1 heading, §H) fills the widgets. This reconciliation is recorded, not assumed silently.
...
- **R-S04** store live only after approval — middleware already routes non-active sellers to `/seller/status`; approval itself is Phase 14 (v1 heading, §H) (admin).
...
mechanism for admin review (Phase 14 (v1 heading, §H) consumes).
...
guidance, EmptyState kit component, seller shell layout). Phase 13 (v1 heading, §H) fills
...
2. E2E LIFECYCLE (staging, minted users, seeded+cleaned, ZERO residue pasted)
   ...
   admin UI is Phase 14 (v1 heading, §H)): confirm approval must flip BOTH
...
6. REGISTER + DOCS SYNC:
   - Close with evidence: REG-10, REG-14 (type-level T01 + runtime exact-
     ...
     the row states the WHY (future multi-writer paths — Phase-14 (v1 heading, §H) admin
   ...
   - SECURITY_GUIDELINES storage section — ONE write folding THREE pieces:
     ...
     PII-lifecycle posture, admin review (Phase 14 (v1 heading, §H)) should know old paths
