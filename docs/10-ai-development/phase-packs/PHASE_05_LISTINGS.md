...
> ships OMITTED (no dead route — the Phase-04 /seller landing-CTA precedent), wired in Phase 11 (v1 heading, §H).
...
- `restock_alerts` — RLS-enabled, ZERO policies, **owned by Phase 12 (v1 heading, §H) (notifications)** per the ERD §3 map. Phase 05
...
- Boost-related register work: NONE (Phase 11 (v1 heading, §H)).
...
- Consumers coming in Phase 05: Listings Management table/tabs, Create/Edit
  Listing form, Stock & Inventory. Boosts are Phase 11 (v1 heading, §H) — nothing boost-styled.
...
restock_alerts (expect ZERO policies — Phase-12 (v1 heading, §H)-owned, VERIFY + DO NOT FIX);
...
- updateStock (consumed by T05): sets stock_qty; R-L07 — CONFIRMED no
  ...
  restock ALERT dispatch (R-N06) is Phase 12 (v1 heading, §H) — do NOT touch
  restock_alerts.
...
BOOST action OMITTED (Phase 11 (v1 heading, §H) — no dead route; state in close-out).
...
restock_alerts is RLS-default-deny and Phase-12 (v1 heading, §H)-owned — render WITHOUT the
count, one code comment citing the deferral + Phase 12 (v1 heading, §H); do not add a
...
1. DoD LEDGER (PASS/FAIL + evidence per line): R-L01..04 publish gates
   ...
   search_vector maintained; boost surfaces ABSENT (Phase 11 (v1 heading, §H), stated);
...
5. REGISTER + DOCS: REG-15/34 closed w/ evidence; REG-26 restated
   ...
   header ("boost" → Phase 11 (v1 heading, §H), the BETK_PHASES drift); PHASE-06 ENTRY
   CHECKLIST written (inquiries/inquiry_messages policies per the ERD
   map; confirm→checkout enablement contract for Phase 07 (retired; v2 checkout is Phase 11);
...
- Inventory low-stock is derived (OD-1); buyers-waiting deferred to Phase 12 (v1 heading, §H) with a cited comment.
- No boost surface exists (Phase 11 (v1 heading, §H)); no dead routes; nav extended only with live routes.
...
| T05 inventory | Sonnet | ✅ DONE (2026-07-22) | `feature/phase-05-listings` | PASS | `/seller/inventory` — compose-only against the already-integration-tested T02 `getOwnInventory`/`updateStock` (zero action/query changes). **5 repo-state FLAGS applied:** (1) OD-1 low-stock stays DERIVED (`stock_qty<=low_stock_threshold` at render via the shared `StockBadge`+`catalogStockLabels`/`catalogStockRemainingLabel` composition — the exact DS component built for this), no `inventory_alerts`/new query added; (2) R-L09 service rows get NO stock controls (em-dash both stock+threshold cells, no input/button — T03 `stockNotTracked` precedent; `updateStock` already rejects services server-side, T02-proven); (3) R-L07 restock reuses the EXISTING `updateStock` action verbatim (imported by file path, not the barrel — T03/T04 precedent), button label swaps to "Restock" only on a `sold_out` row, same action underneath; (4) NAV — added `inventory` to `SellerChrome`, closing the T03 deliberate deferral (comment updated, deferral stated CLOSED); (5) i18n — reused `seller.listings.columns.*`/`.type.*`/`.filter.*`/`.stockNotTracked`/`.newCta`/`.empty.*` + `catalog.stock.*` verbatim, only genuinely new `seller.inventory.*` copy minted (+13 leaf keys, parity 655→668/668). Made-to-order rows keep the SAME editable input as any product (no invented restriction) — "unlimited" is satisfied by `StockBadge`'s own made-to-order-first derive priority. Two empty states (zero listings vs. services-only store, the latter new copy). "Buyers waiting" count DEFERRED with a cited code comment (Phase 12 (v1 heading, §H), `restock_alerts` RLS-default-deny), no policy added. Zero `ui/*`/`shared/*` edits. Full CI green (typecheck·lint 0 new·4 guards[i18n 668/668]·unit 102/102·build 1 new route both locales) + runtime smoke **20/20 PASS** via a throwaway Node+Playwright staging script (1 minted seller + 5 seeded listings covering every stock state, real forged `@supabase/ssr` session cookie, deleted before commit, zero residue): both locales 200 + correct dir/lang; every stock-state rendering (in-stock/low/sold_out/made-to-order/service) proven live; **inline stock-edit round-trip** (50→7, DB-verified) + **restock flip round-trip** (`sold_out`→`active`, stock 0→10, DB-verified) both proven reachable from the real UI, not just the T02 action layer. |
| T06 exit gate | Opus | ✅ DONE (2026-07-22) | `feature/phase-05-listings` | PASS (PR HELD for human) | Exit verification + PR prep; ZERO feature-code changes (docs-only + one throwaway staging E2E, deleted pre-commit, residue re-queried=0). **DoD ledger all-PASS** (R-L01..04 + R-S09-at-publish + REG-15 both-titles + R-L09 hide&strip + R-L06 trigger + R-L07 restock + R-L10 soft-delete + REG-34 ERD-verbatim + tags/images ≤5 + media own-prefix + search_vector + **boost ABSENT** grep-proven). **Cross-phase E2E** (create→publish→PUBLIC detail+browse→order-confirm→R-L06 sold_out→REG-25 detail-visible/browse-absent→R-L07 restock→R-L10 remove→404) green on staging. **DB (MCP):** ledger 26/26 1:1; listings+children pg_policies ERD-verbatim; `restock_alerts` zero-policy; advisor = exact baseline (14/6/2/4/1), 0 new. **CI:** typecheck·lint(0 new)·4 guards (i18n **668/668**, zod 24)·unit **102/102**·integration **17/17** staging·build **40 routes** both locales. Register: REG-15 FULLY CLOSED + FLAG-2 hand-mirror cleanup note; REG-34 closed; REG-26 restated; Phase-05 entry-checklist "boost" header drift corrected→Phase 11 (v1 heading, §H); REG-22 GoTrue phone-500 note folded; Phase-06 entry checklist written. Non-Phase-05 riders PR-listed: `scripts/dev/set-test-role.ts` (commit `48ec4e2`) + T00-pre-wired `seller.listings.filter.*` keys. **ONE migration `20260721111355` → R5 RLS-smoke MUST fire.** Pushed; human opens+merges. |
